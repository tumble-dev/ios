...
extension APIEndpoint {
    var url: URL {
        AppLogger.shared.info("Using API url -> \(Config.apiUrl)")
    ...
    func urlRequest(authToken: String? = nil) -> URLRequest {
        ...
        if let tumbleEndpoint = self as? TumbleEndpoint {
            let allHeaders = tumbleEndpoint.headers(withAuthToken: authToken)
...
extension TumbleEndpoint: APIEndpoint {
    ...
    var headers: [String: String] {
        return [
            ...
            "User-Agent": "Tumble-iOS/1.0",
            "X-API-Key": Config.apiKey
        ]
    }
    
    func headers(withAuthToken token: String?) -> [String: String] {
